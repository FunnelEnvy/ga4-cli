...
const config = new ConfigManager('ga4');
...
/**
 * Resolves an access token from flag > env > config.
 * Handles both OAuth2 tokens and service account tokens.
 */
export async function resolveAccessToken(flagValue?: string): Promise<string | undefined> {
  if (flagValue) return flagValue;

  const envVal = process.env['GA4_ACCESS_TOKEN'];
  ...
  if (!token) {
    console.error(
      `No credentials found. Provide one via:\n` +
        `  1. --access-token flag\n` +
        `  2. GA4_ACCESS_TOKEN environment variable\n` +
        `  3. ga4 auth login`,
    );
    process.exit(1);
  }
...
export async function startOAuthLogin(clientId: string, clientSecret: string): Promise<void> {
  ...
  authUrl.searchParams.set('scope', GA4_SCOPES);
...
/**
 * Sets up a service account key file.
 */
...
async function refreshOAuthToken(
  ...
  if (!response.ok) {
    throw new Error('Failed to refresh OAuth token. Run: ga4 auth login');
  }
